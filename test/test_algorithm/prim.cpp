...
void prim(int n, ListHead **W, int *D, int *P, int *F){
	int R = 1;
	...
	while(!heap.is_empty()){
		...
		if(F[u] == AVAIL){
			W[u]->for_each([&](ListItem *node){
				...
				if(F[v] == AVAIL && node->cost < D[v]){
					P[v] = u;
					D[v] = node->cost;
					heap.insert(new HeapItem(v, D[v]));
				}
			});
			if(heap.heap_size > 1) heap.print(stdout);
		}
		F[u] = USED;
		delete item;
	}
...
int main(){
	...
	printf("Heap:\n");
	prim(num_vertices, W, D, P, F);
	printf("Nearest: ");
	print_list(num_vertices, P);
	printf("Weight nazo nazo: %d\n", sum_list(num_vertices, D));
